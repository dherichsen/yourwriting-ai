...
(function(){
  ...
  var FAQ = [
    { k: /hallucinat|gate|made up|invent|wrong|check|source|cite|citation|verify|proof|receipt/i, a: "The gate sits between the model and the reader. Every draft is split into claims, each claim is looked up in the files you named, and it only goes forward when each one is supported — repaired from the file, or turned into a question when nothing settles it. A receipt lists every claim and its source line. Detail on the <a href='gate.html'>gate page</a>." },
    { k: /fine.?tun|custom|train|adapter|lora|teach|voice|tone|sound like/i, a: "We tune a thin adapter — about 0.4% of the weights — on the writing your business already sends, so drafts come back in your formats and your voice. Four weeks for one kind of letter, graded blind against the base model before anything ships. See <a href='index.html#how'>how it learns your voice</a>." },
  ...
  var opened = false;
  function open(o){ panel.classList.toggle("open", o); btn.setAttribute("aria-expanded", String(o)); if (o && !opened) { opened = true; add("Hi. Ask about the gate, fine-tuning, what runs on the box, pricing or setup. I'm scripted; a person picks up on the contact page.", "bot"); } if (o) input.focus(); }
